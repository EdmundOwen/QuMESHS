...
namespace OneD_ThomasFermiPoisson
{
    public class OneD_PoissonSolver : Potential_Base
    {
        ...
        */
        ...
        {
            DoubleMatrix lap_mat = -1.0 * Generate_Laplacian();
            return new Band_Data(NMathFunctions.Product(lap_mat, input.vec));
        }

        public Band_Data Calculate_Newton_Step(Band_Data rho_prime, DoubleVector rhs)
        {
            DoubleMatrix lap_mat = -1.0 * Generate_Laplacian();

            // check that lap_mat and rho_prime have the same dimension
            if (rho_prime.Length != lap_mat.Rows)
                throw new Exception("Error - the Laplacian generated by pois_solv has a different order to rho_prime!");

            for (int i = 0; i < rho_prime.Length; i++)
                lap_mat[i, i] += rho_prime.vec[i];

            DoubleLUFact lu_newt_step = new DoubleLUFact(lap_mat);

            return new Band_Data(lu_newt_step.Solve(rhs));
        }
