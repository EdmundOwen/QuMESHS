    ...
    public class OneD_PoissonSolver : Potential_Base
    {
        ...
        */
        ...
        public override Band_Data Calculate_Newton_Step(SpinResolved_Data rho_prime, Band_Data rhs)
        {
            ...
            if (rho_prime.Spin_Summed_Data.Length != lap_mat.Rows)
                throw new Exception("Error - the Laplacian generated by pois_solv has a different order to rho_prime!");

            for (int i = 0; i < rho_prime.Spin_Summed_Data.Length; i++)
                lap_mat[i, i] += rho_prime.Spin_Summed_Data.vec[i];

            DoubleLUFact lu_newt_step = new DoubleLUFact(lap_mat);

            return Physics_Base.q_e * new Band_Data(lu_newt_step.Solve(rhs.vec));
        }
