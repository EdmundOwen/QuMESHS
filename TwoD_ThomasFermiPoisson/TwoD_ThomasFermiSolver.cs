            ...
            for (int i = 0; i < ny; i++)
                for (int j = 0; j < nz; j++)
                    for (int E_step = 0; E_step < no_energy_steps; E_step++)
                    {
                        ...
                        result[i, j, Spin.Up] += Physics_Base.Get_3D_DensityofStates(energy_above_eF, conduction_band_energy[i, j]) * Get_Fermi_Function(energy_above_eF);
                        result[i, j, Spin.Down] += Physics_Base.Get_3D_DensityofStates(energy_above_eF, conduction_band_energy[i, j]) * Get_Fermi_Function(energy_above_eF);
                    }
            ...
            for (int i = 0; i < ny; i++)
                for (int j = 0; j < nz; j++)
                    for (int E_step = 0; E_step < no_energy_steps; E_step++)
                    {
                        ...
                        result[i, j, Spin.Up] += Physics_Base.Get_3D_DensityofStates(-1.0 * energy_below_eF, -1.0 * valence_band[i, j]) * (1.0 - Get_Fermi_Function(energy_below_eF));
                        result[i, j, Spin.Down] += Physics_Base.Get_3D_DensityofStates(-1.0 * energy_below_eF, -1.0 * valence_band[i, j]) * (1.0 - Get_Fermi_Function(energy_below_eF));
                    }
