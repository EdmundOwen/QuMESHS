...
namespace TwoD_ThomasFermiPoisson
{
    public class TwoD_PoissonSolver : Potential_Base
    {
        ...
        /// <summary>
        /// runs the NewtonStep file method (with the rhs and rho_prime previously used) in order to 
        /// update the potential file
        /// </summary>
        public void Update_Potential(double t)
        {
        ...
        public void Create_NewtonStep_File(double top_bc, double split_bc, double split_width, double surface, double bottom_bc, string output_file, double t)
        {
            ...
            sw.WriteLine("DEFINITIONS");
            // this is where the density variable
            sw.WriteLine("\tminus_g_phi");
            sw.WriteLine("\tband_gap");
            sw.WriteLine("\tTRANSFER(\'" + pot_filename + "\', phi)");
            sw.WriteLine();
            // and the tables for carrier and donor densities
            sw.WriteLine("\trhs = TABLE(\'" + dens_filename + "\')");
            sw.WriteLine("\trhs_dop = - 1.0 * TABLE(\'" + densdopent_filename + "\')");
            sw.WriteLine("\trho_prime = TABLE(\'" + densderiv_filename + "\')");
            sw.WriteLine();
            sw.WriteLine("\tt = " + t.ToString() + "\t! Mixing parameter");
            // simulation dimension
            sw.WriteLine("\tly = " + (exp.Dy_Pot * exp.Ny_Pot).ToString());
            sw.WriteLine("\tlz = " + (exp.Dz_Pot * exp.Nz_Pot).ToString());
            sw.WriteLine();
            ...
            sw.WriteLine("EQUATIONS");
            // Poisson's equation
            sw.WriteLine("\tu: div(eps * grad(u)) + rho_prime * u = minus_g_phi \t! Poisson's equation");
            sw.WriteLine();
            // the boundary definitions for the differnet layers
            sw.WriteLine("BOUNDARIES");
            ...
            for (int i = 1; i < exp.Layers.Length; i++)
            {
                ...
                if (exp.Layers[i].Layer_No <= Geom_Tool.Find_Layer_Below_Surface(exp.Layers))
                    sw.WriteLine("\t\tminus_g_phi = rhs + rhs_dop");
                else
                    sw.WriteLine("\t\tminus_g_phi = 0.0");
                sw.WriteLine("\t\teps = " + Layer_Tool.Get_Permitivity(exp.Layers[i].Material));
                sw.WriteLine("\t\tband_gap = " + exp.Layers[i].Band_Gap.ToString());
            ...
            sw.WriteLine("PLOTS");
            sw.WriteLine("\tCONTOUR(u)");
            sw.WriteLine("\tCONTOUR(u) ZOOM (" + exp.Ymin_Dens.ToString() + ", " + exp.Zmin_Dens.ToString() + ", " + ((exp.Ny_Dens - 1) * exp.Dy_Dens).ToString() + ", " + ((exp.Nz_Dens - 1) * exp.Dz_Dens).ToString() + ")");
            sw.WriteLine("\tCONTOUR(minus_g_phi) ZOOM (" + exp.Ymin_Dens.ToString() + ", " + exp.Zmin_Dens.ToString() + ", " + ((exp.Ny_Dens - 1) * exp.Dy_Dens).ToString() + ", " + ((exp.Nz_Dens - 1) * exp.Dz_Dens).ToString() + ")");
            sw.WriteLine("\tELEVATION(u) FROM (0, " + (exp.Zmin_Dens + (exp.Nz_Dens + 1) * exp.Dz_Dens).ToString() + ") TO (0, " + (exp.Zmin_Dens - 2.0 * exp.Dz_Dens).ToString() + ")");
            sw.WriteLine("\tELEVATION(minus_g_phi) FROM (0, " + (exp.Zmin_Dens + (exp.Nz_Dens + 1) * exp.Dz_Dens).ToString() + ") TO (0, " + (exp.Zmin_Dens - 2.0 * exp.Dz_Dens).ToString() + ")");
            sw.WriteLine("\tELEVATION(minus_g_phi) FROM (-" + Math.Abs(1.2 * exp.Ymin_Dens).ToString() + ", well_depth) TO (" + Math.Abs(1.2 * exp.Ymin_Dens).ToString() + ", well_depth)");
            sw.WriteLine("\tELEVATION(u) FROM (-" + Math.Abs(1.2 * exp.Ymin_Dens).ToString() + ", well_depth) TO (" + Math.Abs(1.2 * exp.Ymin_Dens).ToString() + ", well_depth)");

            // generate summary containing residual integrals for x and phi
            string window_function_string;
            if (exp.Ymin_Dens < 0.0)
                window_function_string = "(1.0 - upulse(x + " + (-1.0 * exp.Ymin_Dens).ToString() + ", ";
            else
                window_function_string= "(1.0 - upulse(x - " + exp.Ymin_Dens.ToString() + ", ";
            double xmax = exp.Ymin_Dens + exp.Ny_Dens * exp.Dy_Dens;
            if (xmax < 0.0)
                window_function_string = window_function_string + "x + " + (-1.0 * xmax).ToString() + ")";
            else
                window_function_string = window_function_string + "x - " + xmax.ToString() + ")";
            if (exp.Zmin_Dens < 0.0)
                window_function_string = window_function_string + " * upulse(y + " + (-1.0 * exp.Zmin_Dens).ToString() + ", ";
            else
                window_function_string = window_function_string + " * upulse(y - " + exp.Zmin_Dens.ToString() + ", ";
            double ymax = exp.Zmin_Dens + exp.Nz_Dens * exp.Dz_Dens;
            if (ymax < 0.0)
                window_function_string = window_function_string + "y + " + (-1.0 * ymax).ToString() + "))";
            else
                window_function_string = window_function_string + "y - " + ymax.ToString() + "))";

            string residual_report_filename = "residual_g.dat";
            sw.WriteLine("\tSUMMARY EXPORT FILE = \'" + residual_report_filename + "\'");
            sw.WriteLine("\t\tREPORT ( INTEGRAL ((div (eps * grad(phi)) - rhs_dop) * u * " + window_function_string + ")) AS \"residual_g_phi\"");
            sw.WriteLine("\t\tREPORT ( INTEGRAL ((div (eps * grad(u)) - rhs_dop) * u * " + window_function_string + ")) AS \"residual_g_x\"");

            // and transfer the data to a file for reloading and replotting later
            sw.WriteLine();
            sw.WriteLine("\tTABLE(u) ZOOM (" + exp.Ymin_Dens.ToString() + ", " + exp.Zmin_Dens.ToString() + ", " + ((exp.Ny_Dens - 1) * exp.Dy_Dens).ToString() + ", " + ((exp.Nz_Dens - 1) * exp.Dz_Dens).ToString() + ") EXPORT FORMAT \"#1\" POINTS = (" + exp.Ny_Dens.ToString() + ", " + exp.Nz_Dens.ToString() + ") FILE = \"x.dat\"");
            sw.WriteLine("\tTRANSFER(phi + t * u) FILE = \'" + pot_filename + "\'");
            sw.WriteLine();
