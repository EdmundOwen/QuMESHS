...
namespace Solver_Bases
{
    public abstract class Potential_Base
    {
        ...
        /// ALSO: checks for convergence here
        /// </summary>
        public void New_Blend(ref Band_Data band_energy, Band_Data charge_dens_diff, Band_Data charge_dens_deriv, ILayer[] layers, int nz, double dz, double zmin)
        {
            Band_Data old_band_energy = band_energy;

            // check that charge density and chemical potential vectors are of the same lenght
            if (band_energy.Length != charge_dens_diff.Length)
                throw new ArgumentException("Error - band energy and charge density vectors are not of the same length");

            //for (int i = 0; i < band_energy.Length; i++)
            //    band_energy[i] = old_band_energy[i] - charge_dens[i] / charge_dens_deriv[i];

            DoubleMatrix lapmat = Generate_Laplacian(layers, nz, dz, zmin);
            for (int i = 0; i < band_energy.Length; i++)
                lapmat[i, i] -= charge_dens_deriv.vec[i];

            DoubleLUFact lufact = new DoubleLUFact(lapmat);
            Band_Data band_energy_update = new Band_Data(lufact.Solve(-1.0 * charge_dens_diff.vec));

            // check for convergence
            converged = Check_Convergence(band_energy_update, tol);

            band_energy = band_energy + alpha * band_energy_update;
        }

        /// <summary>
        /// Generates a laplacian matrix in one-dimension on a regular grid with Dirichlet BCs wot varying permitivity
        /// </summary>
        DoubleMatrix Generate_Laplacian(ILayer[] layers, int nz, double dz, double zmin)
        {
